            ...
            <div class="search_service">
                ...
                <div class="search_conditions" >
                    ...
                    <select name="searchCondition" class="select" title="조회조건 선택">
                    	<!-- option태그의 selected는 우선선택값 - 기본값 (조회조건의 선택기본값은 선택하세요임-->
			           <option selected value=''>--선택하세요--</option>
			           <!-- 검색조건이(searchCondition) qestnSj로 선택됐다면  -->
			           <option value="qestnSj"  <c:if test="${searchVO.searchCondition == 'qestnSj'}">selected="selected"</c:if> >질문제목</option>            
			       </select>
			       <!-- 검색조건 부분 끝!-->
			       <!-- 검색어 부분 searchKeyword -->
			       		<!-- 인풋태그 타입은 text, 사이즈는 35, 최대글자수maxlength 35, 타이틀은  검색어 입력, 이름은 searchKeyword, 출력은 searchVO의 searchKeyword-->
			       <input name="searchKeyword" type="text" size="35" value='<c:out value="${searchVO.searchKeyword}"/>'  maxlength="35" title="검색어 입력" > 
               	   <!-- 검색어 부분 searchKeyword 끝!-->
                </div>
                	<!-- 검색버튼,등록버튼 -->
                    <div class="search_buttons">
                    	<!-- 검색버튼, 표기되는 이름은 조회. 클릭시 fn_egov_search_faq()동작. false는 어디로 리턴되는 걸까? -->
                        <input type="submit" value="조회" onclick="fn_egov_search_faq(); return false;" />
                        <!-- fn_egov_search_faq 동작시 pageIndex.value = 1; document.FaqListForm.submit();-->
                        <!-- 다시 동작하고 페이지번호 1주는듯 -->
                        
                        <!-- 등록버튼 -->
                        <!--a태그 -> 누르면 fn_egov_regist_faq()동작 - FaqListForm.action = "<c:url value='/uss/olh/faq/FaqCnRegistView.do'/>"; 등록으로 가짐 -->
                        <!-- 누르면 spring message태그로 뭔가 되는듯 -->
